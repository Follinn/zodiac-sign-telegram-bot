...
async def command_start(message : types.Message):
	await bot.send_message(message.from_user.id, 'Привет, я бот проверки на совместимость по знаку зодиака. Напиши свой Знак Зодиака и Знак Зодиака партнёра с которым хочешь проверить совеместимость.\n \nОбязательно что бы знак женщины был слева, иначе совместимость может быть расчитана не правильно \n \nПРИМЕР водолей + овен. ПРОБЕЛ между + и знаками зодиака ОБЯЗАТЕЛЕН')
