...
async def oven(message : types.Message):
    ...
    if message.text.lower() == 'овен + овен':
        ...
        await bot.send_message(message.from_user.id, 'Совместимы на 72%. Она — мягкая, но упрямая, он — строптивый, порывистый, страстный. Он вполне может показать ей, какая энергия прячется внутри нее самой. Но нужно ли ей это?', parse_mode=types.ParseMode.HTML)    
    elif message.text.lower() == 'телец + телец':
        await bot.send_message(message.from_user.id, 'Совместимы на 87%. Они стоят друг друга. Наконец-то у каждого их них нашелся достойный друг. Или соперник?', parse_mode=types.ParseMode.HTML)
    elif message.text.lower() == 'телец + близнецы':
        await bot.send_message(message.from_user.id, 'Совместимы на 73%. Она – приземленная и очень практичная, он – легкий и ветренный. Сумеет ли он надолго задержаться рядом с ней?', parse_mode=types.ParseMode.HTML)
